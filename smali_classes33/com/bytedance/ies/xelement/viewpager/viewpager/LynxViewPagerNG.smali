.class public final Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;
.super Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;
.source "SourceFile"


# annotations
.annotation system Ldalvik/annotation/Signature;
    value = {
        "Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager<",
        "LX/13F8;",
        "LX/13F7;",
        ">;"
    }
.end annotation


# instance fields
.field public LLIZ:Z

.field public LLIZLLLIL:F

.field public LLJ:Z

.field public LLJI:Z

.field public LLJIJIL:I

.field public LLJILJIL:I

.field public LLJILJILJ:Z


# direct methods
.method public static constructor <clinit>()V
    .locals 0

    return-void
.end method

.method public constructor <init>()V
    .locals 1

    const/4 v0, 0x0

    throw v0
.end method

.method public constructor <init>(LX/109i;Ljava/lang/Object;)V
    .locals 1

    invoke-direct {p0, p1, p2}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;-><init>(LX/109i;Ljava/lang/Object;)V

    const/4 v0, 0x1

    iput-boolean v0, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJ:Z

    const/4 v0, -0x1

    iput v0, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJILJIL:I

    return-void
.end method


# virtual methods
.method public final LJJIJLIJ(Lcom/bytedance/ies/xelement/viewpager/childitem/LynxViewpagerItem;I)V
    .locals 3

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v2

    check-cast v2, LX/13F7;

    const/4 v0, 0x1

    invoke-virtual {v2, v0}, LX/13F2;->setMChanged(Z)V

    invoke-virtual {v2}, LX/13F2;->getMTabLayout()LX/12w1;

    move-result-object v0

    if-eqz v0, :cond_0

    iget-object v1, v2, LX/13F7;->LLJLLIL:Ljava/lang/Boolean;

    sget-object v0, Ljava/lang/Boolean;->TRUE:Ljava/lang/Boolean;

    invoke-static {v1, v0}, Lkotlin/jvm/internal/Intrinsics;->LJFF(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_0

    add-int/lit8 p2, p2, -0x1

    :cond_0
    if-ltz p2, :cond_1

    invoke-virtual {v2}, LX/13F2;->getMPendingChildren()Ljava/util/List;

    move-result-object v0

    invoke-interface {v0}, Ljava/util/List;->size()I

    move-result v0

    if-gt p2, v0, :cond_1

    invoke-virtual {v2}, LX/13F2;->getMPendingChildren()Ljava/util/List;

    move-result-object v0

    invoke-static {v0, p2, p1}, Lcom/bytedance/mt/protector/impl/collections/ListProtector;->add(Ljava/util/List;ILjava/lang/Object;)V

    return-void

    :cond_1
    invoke-virtual {v2}, LX/13F2;->getMPendingChildren()Ljava/util/List;

    move-result-object v0

    invoke-interface {v0, p1}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    return-void
.end method

.method public final LJJIZ(Landroid/content/Context;)V
    .locals 2

    invoke-super {p0, p1}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIZ(Landroid/content/Context;)V

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    invoke-virtual {v0}, LX/13F2;->getMViewPager()LX/13F9;

    move-result-object v1

    check-cast v1, LX/13F8;

    new-instance v0, LX/13F6;

    invoke-direct {v0, p0}, LX/13F6;-><init>(Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;)V

    invoke-virtual {v1, v0}, LX/13F8;->setMTouchHandler(LX/13FJ;)V

    return-void
.end method

.method public final LJJJ()V
    .locals 3

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    invoke-virtual {v0}, LX/13F2;->getMViewPager()LX/13F9;

    move-result-object v2

    new-instance v1, LX/1478;

    const/4 v0, 0x1

    invoke-direct {v1, p0, v0}, LX/1478;-><init>(Ljava/lang/Object;I)V

    invoke-virtual {v2, v1}, Landroidx/viewpager/widget/ViewPager;->addOnPageChangeListener(LX/0MSE;)V

    return-void
.end method

.method public final LJJJI(I)V
    .locals 1

    iget v0, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJIJIL:I

    if-eq v0, p1, :cond_0

    iput p1, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJIJIL:I

    const-string v0, ""

    invoke-virtual {p0, p1, v0, v0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJJIL(ILjava/lang/String;Ljava/lang/String;)V

    :cond_0
    return-void
.end method

.method public final LJJJIL(ILjava/lang/String;Ljava/lang/String;)V
    .locals 4

    iget-object v0, p0, Lcom/lynx/tasm/behavior/ui/LynxBaseUI;->mContext:LX/109i;

    iget-object v3, v0, LX/109i;->LLILZ:LX/10Bg;

    new-instance v2, LX/0tGF;

    invoke-virtual {p0}, Lcom/lynx/tasm/behavior/ui/LynxBaseUI;->getSign()I

    move-result v1

    const-string v0, "change"

    invoke-direct {v2, v1, v0}, LX/0tGF;-><init>(ILjava/lang/String;)V

    iget-boolean v0, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLIZ:Z

    invoke-static {v0}, Ljava/lang/Boolean;->valueOf(Z)Ljava/lang/Boolean;

    move-result-object v1

    const-string v0, "isDragged"

    invoke-virtual {v2, v1, v0}, LX/0tGE;->LIZIZ(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v1, "index"

    invoke-static {p1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v0

    invoke-virtual {v2, v0, v1}, LX/0tGE;->LIZIZ(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-virtual {v3, v2}, LX/10Be;->LIZ(LX/0tGE;)V

    return-void
.end method

.method public final LJJJJ(IZ)V
    .locals 4

    iget-object v0, p0, Lcom/lynx/tasm/behavior/ui/LynxBaseUI;->mContext:LX/109i;

    iget-object v3, v0, LX/109i;->LLILZ:LX/10Bg;

    new-instance v2, LX/0tGF;

    invoke-virtual {p0}, Lcom/lynx/tasm/behavior/ui/LynxBaseUI;->getSign()I

    move-result v1

    const-string v0, "willchange"

    invoke-direct {v2, v1, v0}, LX/0tGF;-><init>(ILjava/lang/String;)V

    const-string v1, "isDragged"

    invoke-static {p2}, Ljava/lang/Boolean;->valueOf(Z)Ljava/lang/Boolean;

    move-result-object v0

    invoke-virtual {v2, v0, v1}, LX/0tGE;->LIZIZ(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v1, "index"

    invoke-static {p1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v0

    invoke-virtual {v2, v0, v1}, LX/0tGE;->LIZIZ(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-virtual {v3, v2}, LX/10Be;->LIZ(LX/0tGE;)V

    return-void
.end method

.method public final createView(Landroid/content/Context;)Landroid/view/View;
    .locals 1

    if-nez p1, :cond_0

    const/4 v0, 0x0

    return-object v0

    :cond_0
    new-instance v0, LX/13F7;

    invoke-direct {v0, p1}, LX/13F7;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LLILZIL:LX/13F2;

    invoke-virtual {p0, p1}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIZ(Landroid/content/Context;)V

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    return-object v0
.end method

.method public final isScrollContainer()Z
    .locals 1

    const/4 v0, 0x1

    return v0
.end method

.method public final onNodeReady()V
    .locals 3

    invoke-super {p0}, Lcom/lynx/tasm/behavior/ui/LynxBaseUI;->onNodeReady()V

    iget-boolean v0, p0, Lcom/lynx/tasm/behavior/ui/LynxUI;->mFirstRender:Z

    const/4 v2, 0x0

    if-eqz v0, :cond_0

    iget v1, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJILJIL:I

    const/4 v0, -0x1

    if-eq v1, v0, :cond_0

    if-ltz v1, :cond_0

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    invoke-virtual {v0}, LX/13F2;->getMViewPager()LX/13F9;

    move-result-object v0

    invoke-virtual {v0}, Landroidx/viewpager/widget/ViewPager;->getAdapter()Landroidx/viewpager/widget/PagerAdapter;

    move-result-object v0

    if-eqz v0, :cond_0

    invoke-virtual {v0}, Landroidx/viewpager/widget/PagerAdapter;->getCount()I

    move-result v0

    if-ge v1, v0, :cond_0

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v1

    iget v0, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJILJIL:I

    invoke-virtual {v1, v0}, LX/13F2;->setCurrentSelectIndex(I)V

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    invoke-virtual {v0}, LX/13F2;->getMViewPager()LX/13F9;

    move-result-object v0

    invoke-virtual {v0}, Landroidx/viewpager/widget/ViewPager;->getCurrentItem()I

    move-result v0

    iput v0, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJIJIL:I

    :cond_0
    iput-boolean v2, p0, Lcom/lynx/tasm/behavior/ui/LynxUI;->mFirstRender:Z

    return-void
.end method

.method public final onNodeReload()V
    .locals 2

    invoke-super {p0}, Lcom/lynx/tasm/behavior/ui/LynxBaseUI;->onNodeReload()V

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    iget-object v1, v0, LX/13F2;->LL:LX/13F9;

    const/4 v0, 0x0

    invoke-virtual {v1, v0, v0}, Landroidx/viewpager/widget/ViewPager;->setCurrentItem(IZ)V

    return-void
.end method

.method public selectTab(Lcom/lynx/react/bridge/ReadableMap;Lcom/lynx/react/bridge/Callback;)V
    .locals 10
    .annotation runtime LX/0BCo;
    .end annotation

    new-instance v4, Lcom/lynx/react/bridge/JavaOnlyMap;

    invoke-direct {v4}, Lcom/lynx/react/bridge/JavaOnlyMap;-><init>()V

    sget-object v0, Ljava/lang/Boolean;->FALSE:Ljava/lang/Boolean;

    const-string v6, "success"

    invoke-virtual {v4, v6, v0}, Ljava/util/HashMap;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    const-string v9, "index"

    invoke-interface {p1, v9}, Lcom/lynx/react/bridge/ReadableMap;->hasKey(Ljava/lang/String;)Z

    move-result v7

    const/4 v0, 0x4

    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v8

    const-string v1, "msg"

    const/4 v5, 0x2

    const/4 v3, 0x1

    const/4 v2, 0x0

    if-eqz v7, :cond_4

    invoke-interface {p1, v9}, Lcom/lynx/react/bridge/ReadableMap;->getInt(Ljava/lang/String;)I

    move-result v7

    if-ltz v7, :cond_3

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    invoke-virtual {v0}, LX/13F2;->getMViewPager()LX/13F9;

    move-result-object v0

    invoke-virtual {v0}, Landroidx/viewpager/widget/ViewPager;->getAdapter()Landroidx/viewpager/widget/PagerAdapter;

    move-result-object v0

    if-eqz v0, :cond_3

    invoke-virtual {v0}, Landroidx/viewpager/widget/PagerAdapter;->getCount()I

    move-result v0

    if-ge v7, v0, :cond_3

    invoke-virtual {p0, v7, v2}, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LJJJJ(IZ)V

    const-string v1, "smooth"

    invoke-interface {p1, v1}, Lcom/lynx/react/bridge/ReadableMap;->hasKey(Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_2

    invoke-interface {p1, v1, v3}, Lcom/lynx/react/bridge/ReadableMap;->getBoolean(Ljava/lang/String;Z)Z

    move-result v1

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    iget-object v0, v0, LX/13F2;->LL:LX/13F9;

    invoke-virtual {v0, v7, v1}, Landroidx/viewpager/widget/ViewPager;->setCurrentItem(IZ)V

    if-nez v1, :cond_0

    iget v1, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJIJIL:I

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    invoke-virtual {v0}, LX/13F2;->getMViewPager()LX/13F9;

    move-result-object v0

    invoke-virtual {v0}, Landroidx/viewpager/widget/ViewPager;->getCurrentItem()I

    move-result v0

    if-eq v1, v0, :cond_0

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    invoke-virtual {v0}, LX/13F2;->getMViewPager()LX/13F9;

    move-result-object v0

    invoke-virtual {v0}, Landroidx/viewpager/widget/ViewPager;->getCurrentItem()I

    move-result v1

    iput v1, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJIJIL:I

    const-string v0, ""

    invoke-virtual {p0, v1, v0, v0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJJIL(ILjava/lang/String;Ljava/lang/String;)V

    :cond_0
    :goto_0
    sget-object v0, Ljava/lang/Boolean;->TRUE:Ljava/lang/Boolean;

    invoke-virtual {v4, v6, v0}, Ljava/util/HashMap;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    if-eqz p2, :cond_1

    new-array v1, v5, [Ljava/lang/Object;

    invoke-static {v2}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v0

    aput-object v0, v1, v2

    aput-object v4, v1, v3

    invoke-interface {p2, v1}, Lcom/lynx/react/bridge/Callback;->invoke([Ljava/lang/Object;)V

    :cond_1
    return-void

    :cond_2
    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    iget-object v0, v0, LX/13F2;->LL:LX/13F9;

    invoke-virtual {v0, v7, v3}, Landroidx/viewpager/widget/ViewPager;->setCurrentItem(IZ)V

    goto :goto_0

    :cond_3
    const-string v0, "index out of bounds"

    invoke-virtual {v4, v1, v0}, Ljava/util/HashMap;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    if-eqz p2, :cond_1

    new-array v0, v5, [Ljava/lang/Object;

    aput-object v8, v0, v2

    aput-object v4, v0, v3

    invoke-interface {p2, v0}, Lcom/lynx/react/bridge/Callback;->invoke([Ljava/lang/Object;)V

    return-void

    :cond_4
    const-string v0, "no index key"

    invoke-virtual {v4, v1, v0}, Ljava/util/HashMap;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    if-eqz p2, :cond_1

    new-array v0, v5, [Ljava/lang/Object;

    aput-object v8, v0, v2

    aput-object v4, v0, v3

    invoke-interface {p2, v0}, Lcom/lynx/react/bridge/Callback;->invoke([Ljava/lang/Object;)V

    return-void
.end method

.method public final setAndroidAlwaysOverscroll(Z)V
    .locals 1
    .annotation runtime LX/16wn;
        defaultBoolean = false
        name = "android-always-overscroll"
    .end annotation

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    check-cast v0, LX/13F7;

    invoke-virtual {v0, p1}, LX/13F7;->setViewPagerOverScrollMode(Z)V

    return-void
.end method

.method public final setDistinguishSwipeTap(Z)V
    .locals 0
    .annotation runtime LX/16wn;
        defaultBoolean = true
        name = "android-distinguish-swipe-tap"
    .end annotation

    iput-boolean p1, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJ:Z

    return-void
.end method

.method public final setDragGesture(Lcom/lynx/react/bridge/ReadableMap;Lcom/lynx/react/bridge/Callback;)V
    .locals 4
    .annotation runtime LX/0BCo;
    .end annotation

    new-instance v3, Lcom/lynx/react/bridge/JavaOnlyMap;

    invoke-direct {v3}, Lcom/lynx/react/bridge/JavaOnlyMap;-><init>()V

    const-string v1, "success"

    sget-object v0, Ljava/lang/Boolean;->FALSE:Ljava/lang/Boolean;

    invoke-virtual {v3, v1, v0}, Ljava/util/HashMap;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    const-string v1, "canDrag"

    invoke-interface {p1, v1}, Lcom/lynx/react/bridge/ReadableMap;->hasKey(Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_0

    invoke-interface {p1, v1}, Lcom/lynx/react/bridge/ReadableMap;->getBoolean(Ljava/lang/String;)Z

    move-result v0

    invoke-virtual {p0, v0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->setAllowHorizontalGesture(Z)V

    :cond_0
    if-eqz p2, :cond_1

    const/4 v0, 0x2

    new-array v2, v0, [Ljava/lang/Object;

    const/4 v1, 0x0

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v0

    aput-object v0, v2, v1

    const/4 v0, 0x1

    aput-object v3, v2, v0

    invoke-interface {p2, v2}, Lcom/lynx/react/bridge/Callback;->invoke([Ljava/lang/Object;)V

    :cond_1
    return-void
.end method

.method public final setEnableScroll(Z)V
    .locals 1
    .annotation runtime LX/16wn;
        name = "enable-scroll"
    .end annotation

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    invoke-virtual {v0, p1}, LX/13F2;->setAllowHorizontalGesture(Z)V

    return-void
.end method

.method public final setForceCanScroll(Z)V
    .locals 1
    .annotation runtime LX/16wn;
        name = "android-force-can-scroll"
    .end annotation

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    check-cast v0, LX/13F7;

    invoke-virtual {v0, p1}, LX/13F7;->setForceCanScroll(Z)V

    return-void
.end method

.method public final setInitialSelect(I)V
    .locals 0
    .annotation runtime LX/16wn;
        name = "initial-select-index"
    .end annotation

    iput p1, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJILJIL:I

    return-void
.end method

.method public final setOverflow(I)V
    .locals 2

    invoke-super {p0, p1}, Lcom/lynx/tasm/behavior/ui/LynxBaseUI;->setOverflow(I)V

    const/4 v1, 0x1

    if-eq p1, v1, :cond_0

    const/4 v1, 0x0

    :cond_0
    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    invoke-virtual {v0, v1}, Landroid/view/ViewGroup;->setClipChildren(Z)V

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    invoke-virtual {v0}, LX/13F2;->getMViewPager()LX/13F9;

    move-result-object v0

    invoke-virtual {v0, v1}, Landroid/view/ViewGroup;->setClipChildren(Z)V

    return-void
.end method

.method public final setPageChangeAnimation(Z)V
    .locals 1
    .annotation runtime LX/16wn;
        name = "page-change-animation"
    .end annotation

    invoke-virtual {p0}, Lcom/bytedance/ies/xelement/viewpager/viewpager/BaseLynxViewPager;->LJJIL()LX/13F2;

    move-result-object v0

    check-cast v0, LX/13F7;

    invoke-virtual {v0, p1}, LX/13F7;->setPagerChangeAnimation(Z)V

    return-void
.end method

.method public final setSinglePageTouch(Z)V
    .locals 0
    .annotation runtime LX/16wn;
        name = "single-page-touch"
    .end annotation

    iput-boolean p1, p0, Lcom/bytedance/ies/xelement/viewpager/viewpager/LynxViewPagerNG;->LLJI:Z

    return-void
.end method
