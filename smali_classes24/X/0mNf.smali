.class public final synthetic LX/0mNf;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements LX/0mOM;


# annotations
.annotation runtime LX/05TW;
.end annotation

.annotation system Ldalvik/annotation/EnclosingClass;
    value = Lcom/ss/ugc/effectplatform/model/DiffEffect;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x1001
    name = "a"
.end annotation

.annotation system Ldalvik/annotation/Signature;
    value = {
        "Ljava/lang/Object;",
        "LX/0mOM<",
        "Lcom/ss/ugc/effectplatform/model/DiffEffect;",
        ">;"
    }
.end annotation


# static fields
.field public static final LIZ:LX/0mNf;

.field public static final descriptor:LX/0mPI;


# direct methods
.method public static constructor <clinit>()V
    .locals 4

    new-instance v3, LX/0mNf;

    invoke-direct {v3}, LX/0mNf;-><init>()V

    sput-object v3, LX/0mNf;->LIZ:LX/0mNf;

    new-instance v2, LX/0mOW;

    const-string v1, "com.ss.ugc.effectplatform.model.DiffEffect"

    const/4 v0, 0x3

    invoke-direct {v2, v1, v3, v0}, LX/0mOW;-><init>(Ljava/lang/String;LX/0mOM;I)V

    const-string v0, "diff_url"

    const/4 v1, 0x0

    invoke-virtual {v2, v0, v1}, LX/0mOW;->LJIIIZ(Ljava/lang/String;Z)V

    const-string v0, "base_url"

    invoke-virtual {v2, v0, v1}, LX/0mOW;->LJIIIZ(Ljava/lang/String;Z)V

    const-string v1, "extra"

    const/4 v0, 0x1

    invoke-virtual {v2, v1, v0}, LX/0mOW;->LJIIIZ(Ljava/lang/String;Z)V

    sput-object v2, LX/0mNf;->descriptor:LX/0mPI;

    return-void
.end method

.method public constructor <init>()V
    .locals 0

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method


# virtual methods
.method public final childSerializers()[LX/0mPT;
    .locals 3
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()[",
            "LX/0mPT<",
            "*>;"
        }
    .end annotation

    const/4 v0, 0x3

    new-array v2, v0, [LX/0mPT;

    sget-object v1, Lcom/ss/android/ugc/effectmanager/common/model/UrlModel$$serializer;->INSTANCE:Lcom/ss/android/ugc/effectmanager/common/model/UrlModel$$serializer;

    const/4 v0, 0x0

    aput-object v1, v2, v0

    const/4 v0, 0x1

    aput-object v1, v2, v0

    sget-object v0, LX/0mR9;->LIZ:LX/0mR9;

    invoke-static {v0}, LX/0mP1;->LIZ(LX/0mPT;)LX/0mPT;

    move-result-object v1

    const/4 v0, 0x2

    aput-object v1, v2, v0

    return-object v2
.end method

.method public final deserialize(LX/0mQk;)Ljava/lang/Object;
    .locals 12

    sget-object v6, LX/0mNf;->descriptor:LX/0mPI;

    invoke-interface {p1, v6}, LX/0mQk;->LIZ(LX/0mPI;)LX/0mPa;

    move-result-object v5

    invoke-interface {v5}, LX/0mPa;->LJIILLIIL()V

    const/4 v4, 0x1

    const/4 v3, 0x0

    const/4 v8, 0x0

    const/4 v7, 0x0

    move-object v9, v8

    move-object v10, v8

    :goto_0
    invoke-interface {v5, v6}, LX/0mPa;->LJI(LX/0mPI;)I

    move-result v2

    const/4 v0, -0x1

    if-eq v2, v0, :cond_3

    if-eqz v2, :cond_1

    if-eq v2, v4, :cond_0

    const/4 v1, 0x2

    if-ne v2, v1, :cond_2

    sget-object v0, LX/0mR9;->LIZ:LX/0mR9;

    invoke-interface {v5, v6, v1, v0, v10}, LX/0mPa;->LIZJ(LX/0mPI;ILX/0mQ4;Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v10

    check-cast v10, Ljava/lang/String;

    or-int/lit8 v7, v7, 0x4

    goto :goto_0

    :cond_0
    sget-object v0, Lcom/ss/android/ugc/effectmanager/common/model/UrlModel$$serializer;->INSTANCE:Lcom/ss/android/ugc/effectmanager/common/model/UrlModel$$serializer;

    invoke-interface {v5, v6, v4, v0, v9}, LX/0mPa;->LJIIIIZZ(LX/0mPI;ILX/0mQ4;Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v9

    check-cast v9, Lcom/ss/android/ugc/effectmanager/common/model/UrlModel;

    or-int/lit8 v7, v7, 0x2

    goto :goto_0

    :cond_1
    sget-object v0, Lcom/ss/android/ugc/effectmanager/common/model/UrlModel$$serializer;->INSTANCE:Lcom/ss/android/ugc/effectmanager/common/model/UrlModel$$serializer;

    invoke-interface {v5, v6, v3, v0, v8}, LX/0mPa;->LJIIIIZZ(LX/0mPI;ILX/0mQ4;Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v8

    check-cast v8, Lcom/ss/android/ugc/effectmanager/common/model/UrlModel;

    or-int/lit8 v7, v7, 0x1

    goto :goto_0

    :cond_2
    new-instance v0, LX/0mNi;

    invoke-direct {v0, v2}, LX/0mNi;-><init>(I)V

    throw v0

    :cond_3
    invoke-interface {v5, v6}, LX/0mPa;->LJIIZILJ(LX/0mPI;)V

    new-instance v6, Lcom/ss/ugc/effectplatform/model/DiffEffect;

    const/4 v11, 0x0

    invoke-direct/range {v6 .. v11}, Lcom/ss/ugc/effectplatform/model/DiffEffect;-><init>(ILcom/ss/android/ugc/effectmanager/common/model/UrlModel;Lcom/ss/android/ugc/effectmanager/common/model/UrlModel;Ljava/lang/String;LX/0lvi;)V

    return-object v6
.end method

.method public final getDescriptor()LX/0mPI;
    .locals 1

    sget-object v0, LX/0mNf;->descriptor:LX/0mPI;

    return-object v0
.end method

.method public final serialize(LX/0mQL;Ljava/lang/Object;)V
    .locals 2

    check-cast p2, Lcom/ss/ugc/effectplatform/model/DiffEffect;

    sget-object v1, LX/0mNf;->descriptor:LX/0mPI;

    invoke-interface {p1, v1}, LX/0mQL;->LIZ(LX/0mPI;)LX/0mPb;

    move-result-object v0

    invoke-static {p2, v0, v1}, Lcom/ss/ugc/effectplatform/model/DiffEffect;->write$Self(Lcom/ss/ugc/effectplatform/model/DiffEffect;LX/0mPb;LX/0mPI;)V

    invoke-interface {v0}, LX/0mPb;->LIZIZ()V

    return-void
.end method

.method public final synthetic typeParametersSerializers()[LX/0mPT;
    .locals 1

    sget-object v0, LX/0mNh;->LIZ:[LX/0mPT;

    return-object v0
.end method
