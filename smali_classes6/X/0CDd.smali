.class public final LX/0CDd;
.super Ljava/lang/Object;
.source "SourceFile"


# instance fields
.field public final LIZ:Landroid/graphics/Path;

.field public final LIZIZ:Landroid/graphics/PointF;

.field public final LIZJ:Landroid/graphics/PointF;

.field public final LIZLLL:Landroid/graphics/PointF;

.field public final LJ:Landroid/graphics/PointF;


# direct methods
.method public constructor <init>()V
    .locals 1

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    new-instance v0, Landroid/graphics/Path;

    invoke-direct {v0}, Landroid/graphics/Path;-><init>()V

    iput-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    new-instance v0, Landroid/graphics/PointF;

    invoke-direct {v0}, Landroid/graphics/PointF;-><init>()V

    iput-object v0, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    new-instance v0, Landroid/graphics/PointF;

    invoke-direct {v0}, Landroid/graphics/PointF;-><init>()V

    iput-object v0, p0, LX/0CDd;->LIZJ:Landroid/graphics/PointF;

    new-instance v0, Landroid/graphics/PointF;

    invoke-direct {v0}, Landroid/graphics/PointF;-><init>()V

    iput-object v0, p0, LX/0CDd;->LIZLLL:Landroid/graphics/PointF;

    new-instance v0, Landroid/graphics/PointF;

    invoke-direct {v0}, Landroid/graphics/PointF;-><init>()V

    iput-object v0, p0, LX/0CDd;->LJ:Landroid/graphics/PointF;

    return-void
.end method

.method public static LJI(Landroid/graphics/Path;DDDDDDDZZ)V
    .locals 48

    move-wide/from16 v12, p3

    move-wide/from16 v14, p1

    move-wide/from16 v10, p9

    const/16 v0, 0xb4

    int-to-double v0, v0

    div-double v32, p13, v0

    const-wide v0, 0x400921fb54442d18L    # Math.PI

    mul-double v32, v32, v0

    invoke-static/range {v32 .. v33}, Ljava/lang/Math;->cos(D)D

    move-result-wide v30

    invoke-static/range {v32 .. v33}, Ljava/lang/Math;->sin(D)D

    move-result-wide v28

    mul-double v8, v14, v30

    mul-double v0, v12, v28

    add-double/2addr v8, v0

    div-double/2addr v8, v10

    neg-double v2, v14

    mul-double v2, v2, v28

    mul-double v0, v12, v30

    add-double/2addr v2, v0

    div-double v2, v2, p11

    move-wide/from16 v34, p5

    mul-double v6, v34, v30

    mul-double v0, p7, v28

    add-double/2addr v6, v0

    div-double/2addr v6, v10

    move-wide/from16 v4, v34

    neg-double v0, v4

    mul-double v0, v0, v28

    mul-double v4, p7, v30

    add-double/2addr v0, v4

    div-double v0, v0, p11

    sub-double v26, v8, v6

    sub-double v24, v2, v0

    add-double v44, v8, v6

    const/4 v4, 0x2

    move v4, v4

    int-to-double v4, v4

    move-wide/from16 v46, v4

    div-double v44, v44, v46

    add-double v22, v2, v0

    div-double v22, v22, v46

    mul-double v20, v26, v26

    mul-double v4, v24, v24

    add-double v20, v20, v4

    const-wide/16 v18, 0x0

    cmpg-double v4, v20, v18

    if-nez v4, :cond_0

    return-void

    :cond_0
    const-wide/high16 v16, 0x3ff0000000000000L    # 1.0

    div-double v16, v16, v20

    const-wide/high16 v4, 0x3fd0000000000000L    # 0.25

    sub-double v16, v16, v4

    cmpg-double v4, v16, v18

    move/from16 v5, p15

    move/from16 v36, p16

    if-gez v4, :cond_1

    invoke-static/range {v20 .. v21}, Ljava/lang/Math;->sqrt(D)D

    move-result-wide v0

    const-wide v2, 0x3ffffff583a53b8eL    # 1.99999

    div-double/2addr v0, v2

    double-to-float v2, v0

    float-to-double v0, v2

    mul-double/2addr v10, v0

    mul-double p11, p11, v0

    move-wide/from16 p1, v14

    move-wide/from16 p3, v12

    move-wide/from16 p5, v34

    move-wide/from16 p9, v10

    move/from16 p15, v5

    move/from16 p16, v36

    invoke-static/range {p0 .. p16}, LX/0CDd;->LJI(Landroid/graphics/Path;DDDDDDDZZ)V

    return-void

    :cond_1
    invoke-static/range {v16 .. v17}, Ljava/lang/Math;->sqrt(D)D

    move-result-wide v16

    mul-double v26, v26, v16

    mul-double v16, v16, v24

    move v5, v5

    move/from16 v4, v36

    if-ne v5, v4, :cond_5

    sub-double v44, v44, v16

    add-double v22, v22, v26

    :goto_0
    sub-double v2, v2, v22

    sub-double v8, v8, v44

    invoke-static {v2, v3, v8, v9}, Ljava/lang/Math;->atan2(DD)D

    move-result-wide v20

    sub-double v0, v0, v22

    sub-double v6, v6, v44

    invoke-static {v0, v1, v6, v7}, Ljava/lang/Math;->atan2(DD)D

    move-result-wide v42

    sub-double v42, v42, v20

    cmpl-double v2, v42, v18

    if-ltz v2, :cond_4

    const/4 v1, 0x1

    :goto_1
    move/from16 v0, v36

    if-eq v0, v1, :cond_2

    const-wide v0, 0x401921fb54442d18L    # 6.283185307179586

    if-lez v2, :cond_3

    sub-double v42, v42, v0

    :cond_2
    :goto_2
    mul-double v44, v44, v10

    mul-double v22, v22, p11

    mul-double v40, v44, v30

    mul-double v0, v22, v28

    sub-double v40, v40, v0

    mul-double v44, v44, v28

    mul-double v22, v22, v30

    add-double v44, v44, v22

    const/4 v0, 0x4

    int-to-double v8, v0

    mul-double v2, v42, v8

    const-wide v0, 0x400921fb54442d18L    # Math.PI

    div-double/2addr v2, v0

    invoke-static {v2, v3}, Ljava/lang/Math;->abs(D)D

    move-result-wide v0

    invoke-static {v0, v1}, Ljava/lang/Math;->ceil(D)D

    move-result-wide v0

    double-to-int v2, v0

    move/from16 v39, v2

    invoke-static/range {v32 .. v33}, Ljava/lang/Math;->cos(D)D

    move-result-wide v37

    invoke-static/range {v32 .. v33}, Ljava/lang/Math;->sin(D)D

    move-result-wide v35

    invoke-static/range {v20 .. v21}, Ljava/lang/Math;->cos(D)D

    move-result-wide v2

    invoke-static/range {v20 .. v21}, Ljava/lang/Math;->sin(D)D

    move-result-wide v33

    neg-double v6, v10

    mul-double v31, v6, v37

    mul-double v18, v31, v33

    mul-double v29, p11, v35

    mul-double v0, v29, v2

    sub-double v18, v18, v0

    mul-double v6, v6, v35

    mul-double v33, v33, v6

    mul-double p11, p11, v37

    mul-double v2, v2, p11

    add-double v33, v33, v2

    move/from16 v0, v39

    int-to-double v0, v0

    div-double v42, v42, v0

    if-lez v39, :cond_6

    const/16 v28, 0x0

    :goto_3
    add-int/lit8 v28, v28, 0x1

    add-double v26, v20, v42

    invoke-static/range {v26 .. v27}, Ljava/lang/Math;->sin(D)D

    move-result-wide v24

    invoke-static/range {v26 .. v27}, Ljava/lang/Math;->cos(D)D

    move-result-wide v16

    mul-double v0, v10, v37

    mul-double v0, v0, v16

    add-double v4, v40, v0

    mul-double v0, v29, v24

    sub-double/2addr v4, v0

    mul-double v0, v10, v35

    mul-double v0, v0, v16

    add-double v2, v44, v0

    mul-double v0, p11, v24

    add-double/2addr v2, v0

    mul-double v22, v31, v24

    mul-double v0, v29, v16

    sub-double v22, v22, v0

    mul-double v24, v24, v6

    mul-double v16, v16, p11

    add-double v24, v24, v16

    sub-double v16, v26, v20

    div-double v0, v16, v46

    invoke-static {v0, v1}, Ljava/lang/Math;->tan(D)D

    move-result-wide v0

    invoke-static/range {v16 .. v17}, Ljava/lang/Math;->sin(D)D

    move-result-wide v20

    const-wide/high16 v16, 0x4008000000000000L    # 3.0

    mul-double v16, v16, v0

    mul-double v16, v16, v0

    add-double v0, v8, v16

    invoke-static {v0, v1}, Ljava/lang/Math;->sqrt(D)D

    move-result-wide v16

    const/4 v0, 0x1

    int-to-double v0, v0

    sub-double v16, v16, v0

    mul-double v20, v20, v16

    const/4 v0, 0x3

    int-to-double v0, v0

    div-double v20, v20, v0

    mul-double v18, v18, v20

    add-double v14, v14, v18

    mul-double v33, v33, v20

    add-double v12, v12, v33

    mul-double v0, v20, v22

    sub-double v18, v4, v0

    mul-double v20, v20, v24

    sub-double v16, v2, v20

    double-to-float v0, v14

    move v15, v0

    double-to-float v0, v12

    move v14, v0

    move-wide/from16 v0, v18

    double-to-float v12, v0

    move-wide/from16 v0, v16

    double-to-float v13, v0

    double-to-float v1, v4

    double-to-float v0, v2

    move/from16 p1, v15

    move/from16 p2, v14

    move/from16 p3, v12

    move/from16 p4, v13

    move/from16 p5, v1

    move/from16 p6, v0

    invoke-virtual/range {p0 .. p6}, Landroid/graphics/Path;->cubicTo(FFFFFF)V

    move/from16 v1, v28

    move/from16 v0, v39

    if-ge v1, v0, :cond_6

    move-wide v14, v4

    move-wide v12, v2

    move-wide/from16 v20, v26

    move-wide/from16 v33, v24

    move-wide/from16 v18, v22

    goto/16 :goto_3

    :cond_3
    add-double v42, v42, v0

    goto/16 :goto_2

    :cond_4
    const/4 v1, 0x0

    goto/16 :goto_1

    :cond_5
    add-double v44, v44, v16

    sub-double v22, v22, v26

    goto/16 :goto_0

    :cond_6
    return-void
.end method


# virtual methods
.method public final LIZ(FFFLandroid/graphics/Path$Direction;)V
    .locals 1

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    invoke-virtual {v0, p1, p2, p3, p4}, Landroid/graphics/Path;->addCircle(FFFLandroid/graphics/Path$Direction;)V

    return-void
.end method

.method public final LIZIZ(FFLandroid/graphics/Path$Direction;)V
    .locals 6

    const/4 v1, 0x0

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    move v2, v1

    move-object v5, p3

    move v4, p2

    move v3, p1

    invoke-virtual/range {v0 .. v5}, Landroid/graphics/Path;->addRect(FFFFLandroid/graphics/Path$Direction;)V

    return-void
.end method

.method public final LIZJ(Landroid/graphics/RectF;FFLandroid/graphics/Path$Direction;)V
    .locals 1

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    invoke-virtual {v0, p1, p2, p3, p4}, Landroid/graphics/Path;->addRoundRect(Landroid/graphics/RectF;FFLandroid/graphics/Path$Direction;)V

    return-void
.end method

.method public final LIZLLL(FFZZFF)V
    .locals 34

    move-object/from16 v14, p0

    iget-object v0, v14, LX/0CDd;->LIZ:Landroid/graphics/Path;

    move-object/from16 v33, v0

    iget-object v1, v14, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->x:F

    float-to-double v15, v0

    iget v0, v1, Landroid/graphics/PointF;->y:F

    float-to-double v10, v0

    move/from16 v13, p5

    float-to-double v8, v13

    move/from16 v12, p6

    float-to-double v6, v12

    move/from16 v0, p1

    float-to-double v4, v0

    move/from16 v0, p2

    float-to-double v2, v0

    const/4 v0, 0x0

    float-to-double v0, v0

    move/from16 v32, p4

    move/from16 v31, p3

    move-wide/from16 v29, v0

    move-wide/from16 v27, v2

    move-wide/from16 v25, v4

    move-wide/from16 v23, v6

    move-wide/from16 v21, v8

    move-wide/from16 v19, v10

    move-wide/from16 v17, v15

    move-object/from16 v16, v33

    invoke-static/range {v16 .. v32}, LX/0CDd;->LJI(Landroid/graphics/Path;DDDDDDDZZ)V

    iget-object v0, v14, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iput v13, v0, Landroid/graphics/PointF;->x:F

    iput v12, v0, Landroid/graphics/PointF;->y:F

    iget-object v0, v14, LX/0CDd;->LIZJ:Landroid/graphics/PointF;

    iput v13, v0, Landroid/graphics/PointF;->x:F

    iput v12, v0, Landroid/graphics/PointF;->y:F

    return-void
.end method

.method public final LJ()V
    .locals 3

    iget-object v1, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget-object v2, p0, LX/0CDd;->LIZLLL:Landroid/graphics/PointF;

    iget v0, v2, Landroid/graphics/PointF;->x:F

    iput v0, v1, Landroid/graphics/PointF;->x:F

    iget v0, v2, Landroid/graphics/PointF;->y:F

    iput v0, v1, Landroid/graphics/PointF;->y:F

    iget-object v1, p0, LX/0CDd;->LIZJ:Landroid/graphics/PointF;

    iget v0, v2, Landroid/graphics/PointF;->x:F

    iput v0, v1, Landroid/graphics/PointF;->x:F

    iget v0, v2, Landroid/graphics/PointF;->y:F

    iput v0, v1, Landroid/graphics/PointF;->y:F

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    invoke-virtual {v0}, Landroid/graphics/Path;->close()V

    iget-object v2, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    iget-object v0, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v1, v0, Landroid/graphics/PointF;->x:F

    iget v0, v0, Landroid/graphics/PointF;->y:F

    invoke-virtual {v2, v1, v0}, Landroid/graphics/Path;->moveTo(FF)V

    return-void
.end method

.method public final LJFF(FFFFFF)V
    .locals 7

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    move v6, p6

    move v5, p5

    move v4, p4

    move v3, p3

    move v2, p2

    move v1, p1

    invoke-virtual/range {v0 .. v6}, Landroid/graphics/Path;->cubicTo(FFFFFF)V

    iget-object v0, p0, LX/0CDd;->LIZJ:Landroid/graphics/PointF;

    iput v3, v0, Landroid/graphics/PointF;->x:F

    iput v4, v0, Landroid/graphics/PointF;->y:F

    iget-object v0, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iput v5, v0, Landroid/graphics/PointF;->x:F

    iput v6, v0, Landroid/graphics/PointF;->y:F

    return-void
.end method

.method public final LJII(F)V
    .locals 2

    iget-object v1, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    iget-object v0, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v0, Landroid/graphics/PointF;->y:F

    invoke-virtual {v1, p1, v0}, Landroid/graphics/Path;->lineTo(FF)V

    iget-object v0, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iput p1, v0, Landroid/graphics/PointF;->x:F

    return-void
.end method

.method public final LJIIIIZZ(FF)V
    .locals 1

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    invoke-virtual {v0, p1, p2}, Landroid/graphics/Path;->lineTo(FF)V

    iget-object v0, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iput p1, v0, Landroid/graphics/PointF;->x:F

    iput p2, v0, Landroid/graphics/PointF;->y:F

    return-void
.end method

.method public final LJIIIZ(FF)V
    .locals 3

    iget-object v0, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iput p1, v0, Landroid/graphics/PointF;->x:F

    iput p2, v0, Landroid/graphics/PointF;->y:F

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    invoke-virtual {v0, p1, p2}, Landroid/graphics/Path;->moveTo(FF)V

    iget-object v2, p0, LX/0CDd;->LIZLLL:Landroid/graphics/PointF;

    iget-object v1, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->x:F

    iput v0, v2, Landroid/graphics/PointF;->x:F

    iget v0, v1, Landroid/graphics/PointF;->y:F

    iput v0, v2, Landroid/graphics/PointF;->y:F

    return-void
.end method

.method public final LJIIJ(FFZZFF)V
    .locals 34

    move/from16 v12, p6

    move/from16 v13, p5

    move-object/from16 v14, p0

    iget-object v0, v14, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v2, v0, Landroid/graphics/PointF;->x:F

    add-float/2addr v13, v2

    iget v1, v0, Landroid/graphics/PointF;->y:F

    add-float/2addr v12, v1

    iget-object v0, v14, LX/0CDd;->LIZ:Landroid/graphics/Path;

    move-object/from16 v33, v0

    float-to-double v15, v2

    float-to-double v10, v1

    float-to-double v8, v13

    float-to-double v6, v12

    move/from16 v0, p1

    float-to-double v4, v0

    move/from16 v0, p2

    float-to-double v2, v0

    const/4 v0, 0x0

    float-to-double v0, v0

    move/from16 v32, p4

    move/from16 v31, p3

    move-wide/from16 v27, v2

    move-wide/from16 v29, v0

    move-wide/from16 v25, v4

    move-wide/from16 v23, v6

    move-wide/from16 v21, v8

    move-wide/from16 v19, v10

    move-wide/from16 v17, v15

    move-object/from16 v16, v33

    invoke-static/range {v16 .. v32}, LX/0CDd;->LJI(Landroid/graphics/Path;DDDDDDDZZ)V

    iget-object v0, v14, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iput v13, v0, Landroid/graphics/PointF;->x:F

    iput v12, v0, Landroid/graphics/PointF;->y:F

    iget-object v0, v14, LX/0CDd;->LIZJ:Landroid/graphics/PointF;

    iput v13, v0, Landroid/graphics/PointF;->x:F

    iput v12, v0, Landroid/graphics/PointF;->y:F

    return-void
.end method

.method public final LJIIJJI(FFFFFF)V
    .locals 7

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    move v6, p6

    move v5, p5

    move v4, p4

    move v3, p3

    move v2, p2

    move v1, p1

    invoke-virtual/range {v0 .. v6}, Landroid/graphics/Path;->rCubicTo(FFFFFF)V

    iget-object v2, p0, LX/0CDd;->LIZJ:Landroid/graphics/PointF;

    iget-object v1, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->x:F

    add-float/2addr v0, v3

    iput v0, v2, Landroid/graphics/PointF;->x:F

    iget v0, v1, Landroid/graphics/PointF;->y:F

    add-float/2addr v0, v4

    iput v0, v2, Landroid/graphics/PointF;->y:F

    iget v0, v1, Landroid/graphics/PointF;->x:F

    add-float/2addr v0, v5

    iput v0, v1, Landroid/graphics/PointF;->x:F

    iget v0, v1, Landroid/graphics/PointF;->y:F

    add-float/2addr v0, v6

    iput v0, v1, Landroid/graphics/PointF;->y:F

    return-void
.end method

.method public final LJIIL(F)V
    .locals 2

    iget-object v1, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    const/4 v0, 0x0

    invoke-virtual {v1, p1, v0}, Landroid/graphics/Path;->rLineTo(FF)V

    iget-object v1, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->x:F

    add-float/2addr v0, p1

    iput v0, v1, Landroid/graphics/PointF;->x:F

    return-void
.end method

.method public final LJIILIIL(FF)V
    .locals 2

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    invoke-virtual {v0, p1, p2}, Landroid/graphics/Path;->rLineTo(FF)V

    iget-object v1, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->x:F

    add-float/2addr v0, p1

    iput v0, v1, Landroid/graphics/PointF;->x:F

    iget v0, v1, Landroid/graphics/PointF;->y:F

    add-float/2addr v0, p2

    iput v0, v1, Landroid/graphics/PointF;->y:F

    return-void
.end method

.method public final LJIILJJIL(FF)V
    .locals 3

    iget-object v1, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->x:F

    add-float/2addr v0, p1

    iput v0, v1, Landroid/graphics/PointF;->x:F

    iget v0, v1, Landroid/graphics/PointF;->y:F

    add-float/2addr v0, p2

    iput v0, v1, Landroid/graphics/PointF;->y:F

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    invoke-virtual {v0, p1, p2}, Landroid/graphics/Path;->rMoveTo(FF)V

    iget-object v2, p0, LX/0CDd;->LIZLLL:Landroid/graphics/PointF;

    iget-object v1, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->x:F

    iput v0, v2, Landroid/graphics/PointF;->x:F

    iget v0, v1, Landroid/graphics/PointF;->y:F

    iput v0, v2, Landroid/graphics/PointF;->y:F

    return-void
.end method

.method public final LJIILL(FFFFZ)V
    .locals 8

    if-eqz p5, :cond_0

    iget-object v4, p0, LX/0CDd;->LJ:Landroid/graphics/PointF;

    iget-object v3, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v1, v3, Landroid/graphics/PointF;->x:F

    iget-object v2, p0, LX/0CDd;->LIZJ:Landroid/graphics/PointF;

    iget v0, v2, Landroid/graphics/PointF;->x:F

    sub-float/2addr v1, v0

    iput v1, v4, Landroid/graphics/PointF;->x:F

    iget v1, v3, Landroid/graphics/PointF;->y:F

    iget v0, v2, Landroid/graphics/PointF;->y:F

    sub-float/2addr v1, v0

    iput v1, v4, Landroid/graphics/PointF;->y:F

    :goto_0
    iget-object v1, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    iget-object v0, p0, LX/0CDd;->LJ:Landroid/graphics/PointF;

    iget v2, v0, Landroid/graphics/PointF;->x:F

    iget v3, v0, Landroid/graphics/PointF;->y:F

    move v7, p4

    move v6, p3

    move v5, p2

    move v4, p1

    invoke-virtual/range {v1 .. v7}, Landroid/graphics/Path;->rCubicTo(FFFFFF)V

    iget-object v2, p0, LX/0CDd;->LIZJ:Landroid/graphics/PointF;

    iget-object v1, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->x:F

    add-float/2addr v0, v4

    iput v0, v2, Landroid/graphics/PointF;->x:F

    iget v0, v1, Landroid/graphics/PointF;->y:F

    add-float/2addr v0, v5

    iput v0, v2, Landroid/graphics/PointF;->y:F

    iget v0, v1, Landroid/graphics/PointF;->x:F

    add-float/2addr v0, v6

    iput v0, v1, Landroid/graphics/PointF;->x:F

    iget v0, v1, Landroid/graphics/PointF;->y:F

    add-float/2addr v0, v7

    iput v0, v1, Landroid/graphics/PointF;->y:F

    return-void

    :cond_0
    iget-object v1, p0, LX/0CDd;->LJ:Landroid/graphics/PointF;

    const/4 v0, 0x0

    iput v0, v1, Landroid/graphics/PointF;->x:F

    iput v0, v1, Landroid/graphics/PointF;->y:F

    goto :goto_0
.end method

.method public final LJIILLIIL(F)V
    .locals 2

    iget-object v1, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    const/4 v0, 0x0

    invoke-virtual {v1, v0, p1}, Landroid/graphics/Path;->rLineTo(FF)V

    iget-object v1, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->y:F

    add-float/2addr v0, p1

    iput v0, v1, Landroid/graphics/PointF;->y:F

    return-void
.end method

.method public final LJIIZILJ(FFFFZ)V
    .locals 8

    if-eqz p5, :cond_0

    iget-object v5, p0, LX/0CDd;->LJ:Landroid/graphics/PointF;

    const/4 v0, 0x2

    int-to-float v4, v0

    iget-object v3, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v2, v3, Landroid/graphics/PointF;->x:F

    mul-float/2addr v2, v4

    iget-object v1, p0, LX/0CDd;->LIZJ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->x:F

    sub-float/2addr v2, v0

    iput v2, v5, Landroid/graphics/PointF;->x:F

    iget v0, v3, Landroid/graphics/PointF;->y:F

    mul-float/2addr v4, v0

    iget v0, v1, Landroid/graphics/PointF;->y:F

    sub-float/2addr v4, v0

    iput v4, v5, Landroid/graphics/PointF;->y:F

    :goto_0
    iget-object v1, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    iget-object v0, p0, LX/0CDd;->LJ:Landroid/graphics/PointF;

    iget v2, v0, Landroid/graphics/PointF;->x:F

    iget v3, v0, Landroid/graphics/PointF;->y:F

    move v7, p4

    move v6, p3

    move v5, p2

    move v4, p1

    invoke-virtual/range {v1 .. v7}, Landroid/graphics/Path;->cubicTo(FFFFFF)V

    iget-object v0, p0, LX/0CDd;->LIZJ:Landroid/graphics/PointF;

    iput v4, v0, Landroid/graphics/PointF;->x:F

    iput v5, v0, Landroid/graphics/PointF;->y:F

    iget-object v0, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iput v6, v0, Landroid/graphics/PointF;->x:F

    iput v7, v0, Landroid/graphics/PointF;->y:F

    return-void

    :cond_0
    iget-object v2, p0, LX/0CDd;->LJ:Landroid/graphics/PointF;

    iget-object v1, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v1, Landroid/graphics/PointF;->x:F

    iput v0, v2, Landroid/graphics/PointF;->x:F

    iget v0, v1, Landroid/graphics/PointF;->y:F

    iput v0, v2, Landroid/graphics/PointF;->y:F

    goto :goto_0
.end method

.method public final LJIJ(Landroid/graphics/Path$FillType;)V
    .locals 1

    iget-object v0, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    invoke-virtual {v0, p1}, Landroid/graphics/Path;->setFillType(Landroid/graphics/Path$FillType;)V

    return-void
.end method

.method public final LJIJI(F)V
    .locals 2

    iget-object v1, p0, LX/0CDd;->LIZ:Landroid/graphics/Path;

    iget-object v0, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iget v0, v0, Landroid/graphics/PointF;->x:F

    invoke-virtual {v1, v0, p1}, Landroid/graphics/Path;->lineTo(FF)V

    iget-object v0, p0, LX/0CDd;->LIZIZ:Landroid/graphics/PointF;

    iput p1, v0, Landroid/graphics/PointF;->y:F

    return-void
.end method
