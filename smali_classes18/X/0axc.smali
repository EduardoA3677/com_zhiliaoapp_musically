.class public final LX/0axc;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements LX/0aur;
.implements LX/02uK;


# static fields
.field public static final synthetic LLILLL:[LX/10fb;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "[",
            "LX/10fb<",
            "Ljava/lang/Object;",
            ">;"
        }
    .end annotation
.end field


# instance fields
.field public final LL:Landroid/content/Context;

.field public final LLILIL:LX/05ta;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "LX/05ta<",
            "Lcom/ss/android/ugc/aweme/im/messagelist/api/IAwemeStatusViewModel;",
            ">;"
        }
    .end annotation
.end field

.field public final LLILL:Z

.field public final LLILLIZIL:LX/0PBG;

.field public volatile LLILLJJLI:LX/03vm;


# direct methods
.method public static constructor <clinit>()V
    .locals 6

    const/4 v0, 0x1

    new-array v5, v0, [LX/10fb;

    new-instance v4, LX/10fW;

    const-class v3, LX/0axc;

    const-string v2, "ttDispatcher"

    const-string v0, "getTtDispatcher()Lcom/ss/android/ugc/aweme/im/saas/host/api/utils/IMDispatchers;"

    const/4 v1, 0x0

    invoke-direct {v4, v3, v2, v0, v1}, LX/10fW;-><init>(Ljava/lang/Class;Ljava/lang/String;Ljava/lang/String;I)V

    sget-object v0, LX/0mTc;->LIZ:LX/0mTZ;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    aput-object v4, v5, v1

    sput-object v5, LX/0axc;->LLILLL:[LX/10fb;

    return-void
.end method

.method public constructor <init>(Landroid/content/Context;Lcom/bytedance/assem/arch/viewModel/AssemVMLazy;Z)V
    .locals 1

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-object p1, p0, LX/0axc;->LL:Landroid/content/Context;

    iput-object p2, p0, LX/0axc;->LLILIL:LX/05ta;

    iput-boolean p3, p0, LX/0axc;->LLILL:Z

    iget-object v0, p0, LX/0axc;->LLILLJJLI:LX/03vm;

    if-nez v0, :cond_1

    monitor-enter p0

    :try_start_0
    iget-object v0, p0, LX/0axc;->LLILLJJLI:LX/03vm;

    if-nez v0, :cond_0

    invoke-static {}, LX/0bId;->LJJIJ()LX/03vm;

    move-result-object v0

    iput-object v0, p0, LX/0axc;->LLILLJJLI:LX/03vm;

    monitor-exit p0

    goto :goto_0
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    :catchall_0
    move-exception v0

    monitor-exit p0

    throw v0

    :cond_0
    monitor-exit p0

    :cond_1
    :goto_0
    iget-object v0, v0, LX/03vm;->LIZ:LX/0PBG;

    iput-object v0, p0, LX/0axc;->LLILLIZIL:LX/0PBG;

    return-void
.end method

.method public static LIZIZ(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;)LX/0avP;
    .locals 14

    sget-object v0, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;->ATTACHMENT_TYPE_SHARE_COMMENT_MENTION:Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;

    move-object/from16 v1, p2

    move-object v13, p0

    if-ne v1, v0, :cond_0

    new-instance v10, LX/0avF;

    new-instance v4, LX/0axl;

    invoke-virtual {p1}, Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;->getRefVideoId()Ljava/lang/String;

    move-result-object v6

    invoke-virtual {v13}, LX/0i9W;->getSender()J

    move-result-wide v0

    invoke-static {v0, v1}, Ljava/lang/String;->valueOf(J)Ljava/lang/String;

    move-result-object v7

    invoke-virtual {p1}, Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;->getRefVideoIsStory()Z

    move-result v0

    invoke-static {v0}, Ljava/lang/Boolean;->valueOf(Z)Ljava/lang/Boolean;

    move-result-object v5

    const/4 v9, 0x0

    invoke-virtual {v13}, LX/0i9W;->getScene()Ljava/lang/String;

    move-result-object v8

    invoke-direct/range {v4 .. v9}, LX/0axl;-><init>(Ljava/lang/Boolean;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Z)V

    invoke-virtual {v13}, LX/0i9W;->getMsgId()J

    move-result-wide v2

    invoke-virtual {v13}, LX/0i9W;->getMsgType()I

    move-result v0

    int-to-long v0, v0

    invoke-static {v0, v1}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v0

    invoke-direct {v10, v4, v2, v3, v0}, LX/0avF;-><init>(LX/0axl;JLjava/lang/Long;)V

    return-object v10

    :cond_0
    invoke-virtual {v13}, LX/0i9W;->getMsgType()I

    move-result v1

    const/16 v0, 0x719

    if-ne v1, v0, :cond_5

    invoke-static {v13}, LX/0atE;->LIZ(LX/0i9W;)Lcom/ss/android/ugc/aweme/im/message/template/card/BaseTemplate;

    move-result-object v2

    check-cast v2, Lcom/ss/android/ugc/aweme/im/message/template/card/ShareRepostCardTemplate;

    new-instance v10, LX/0avB;

    if-eqz v2, :cond_4

    iget-object v0, v2, Lcom/ss/android/ugc/aweme/im/message/template/card/ShareRepostCardTemplate;->attachment:Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;

    if-eqz v0, :cond_4

    iget-object v11, v0, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;->id:Ljava/lang/String;

    :goto_0
    const-string v1, ""

    if-nez v11, :cond_1

    move-object v11, v1

    :cond_1
    if-eqz v2, :cond_2

    iget-object v0, v2, Lcom/ss/android/ugc/aweme/im/message/template/card/ShareRepostCardTemplate;->shareRepost:Lcom/ss/android/ugc/aweme/im/message/template/component/ShareRepostComponent;

    if-eqz v0, :cond_2

    iget-object v0, v0, Lcom/ss/android/ugc/aweme/im/message/template/component/ShareRepostComponent;->repostUserInfo:Lcom/ss/android/ugc/aweme/im/message/template/component/BaseUserComponent;

    if-eqz v0, :cond_2

    iget-object v0, v0, Lcom/ss/android/ugc/aweme/im/message/template/component/BaseUserComponent;->userId:Ljava/lang/Long;

    if-eqz v0, :cond_2

    invoke-virtual {v0}, Ljava/lang/Long;->toString()Ljava/lang/String;

    move-result-object v12

    if-nez v12, :cond_3

    :cond_2
    move-object v12, v1

    :cond_3
    invoke-virtual {v13}, LX/0i9W;->getMsgId()J

    move-result-wide p0

    invoke-virtual {v13}, LX/0i9W;->getMsgType()I

    move-result v0

    int-to-long v0, v0

    invoke-static {v0, v1}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object p2

    invoke-direct/range {v10 .. v16}, LX/0avB;-><init>(Ljava/lang/String;Ljava/lang/String;LX/0i9W;JLjava/lang/Long;)V

    return-object v10

    :cond_4
    const/4 v11, 0x0

    goto :goto_0

    :cond_5
    invoke-static {v13}, LX/0axf;->LIZIZ(LX/0i9W;)Z

    move-result v0

    if-eqz v0, :cond_6

    invoke-virtual {p1}, Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;->getRefVideoId()Ljava/lang/String;

    move-result-object v3

    invoke-virtual {v13}, LX/0i9W;->getRefMsgId()J

    move-result-wide v1

    invoke-static {v13}, LX/0b3L;->LJIIL(LX/0i9W;)Lcom/ss/android/ugc/aweme/im/message/content/TextReactAttachment;

    move-result-object v0

    new-instance v10, LX/0avS;

    invoke-direct {v10, v3, v1, v2, v0}, LX/0avS;-><init>(Ljava/lang/String;JLcom/ss/android/ugc/aweme/im/message/content/TextReactAttachment;)V

    return-object v10

    :cond_6
    new-instance v10, LX/0avQ;

    new-instance v3, LX/0axl;

    invoke-virtual {p1}, Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;->getRefVideoId()Ljava/lang/String;

    move-result-object v2

    invoke-virtual {v13}, LX/0i9W;->getSender()J

    move-result-wide v0

    invoke-static {v0, v1}, Ljava/lang/String;->valueOf(J)Ljava/lang/String;

    move-result-object v1

    invoke-virtual {p1}, Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;->getRefVideoIsStory()Z

    move-result v0

    invoke-static {v0}, Ljava/lang/Boolean;->valueOf(Z)Ljava/lang/Boolean;

    move-result-object v0

    invoke-direct {v3, v2, v1, v0}, LX/0axl;-><init>(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Boolean;)V

    invoke-virtual {p1}, Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;->getRefVideoAwemeType()I

    move-result v2

    invoke-virtual {v13}, LX/0i9W;->getRefMsgId()J

    move-result-wide v0

    invoke-direct {v10, v3, v2, v0, v1}, LX/0avQ;-><init>(LX/0axl;IJ)V

    return-object v10
.end method


# virtual methods
.method public final LIZ(LX/0i9W;)LX/0aup;
    .locals 10
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "LX/0i9W;",
            ")",
            "LX/0aup<",
            "LX/0awt;",
            ">;"
        }
    .end annotation

    invoke-static {p1}, LX/0atE;->LIZ(LX/0i9W;)Lcom/ss/android/ugc/aweme/im/message/template/card/BaseTemplate;

    move-result-object v0

    const/4 v7, 0x0

    if-eqz v0, :cond_b

    invoke-interface {v0}, Lcom/ss/android/ugc/aweme/im/message/template/card/BaseTemplate;->Iq()Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;

    move-result-object v5

    if-eqz v5, :cond_b

    iget-object v0, v5, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;->id:Ljava/lang/String;

    invoke-virtual {v0}, Ljava/lang/String;->length()I

    move-result v1

    const/4 v0, 0x0

    if-eqz v1, :cond_b

    invoke-static {v5}, LX/0b48;->LIZIZ(Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;)Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;

    move-result-object v4

    invoke-virtual {v4}, Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;->getRefVideoId()Ljava/lang/String;

    move-result-object v1

    if-eqz v1, :cond_0

    invoke-static {v0}, LX/078q;->LIZIZ(Z)Lcom/ss/android/ugc/aweme/im/service/IIMService;

    move-result-object v0

    invoke-interface {v0}, Lcom/ss/android/ugc/aweme/im/service/IIMService;->getAwemeFetchManager()Lcom/ss/android/ugc/aweme/im/common/service/IAwemeFetchManager;

    move-result-object v0

    invoke-interface {v0, v1}, Lcom/ss/android/ugc/aweme/im/common/service/IAwemeFetchManager;->LIZLLL(Ljava/lang/String;)Lcom/ss/android/ugc/aweme/base/model/UrlModel;

    move-result-object v6

    if-nez v6, :cond_1

    :cond_0
    invoke-virtual {v4}, Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;->getRefVideoCoverUrl()Lcom/ss/android/ugc/aweme/base/model/UrlModel;

    move-result-object v6

    :cond_1
    iget-object v0, p0, LX/0axc;->LLILIL:LX/05ta;

    invoke-interface {v0}, LX/05ta;->getValue()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/ss/android/ugc/aweme/im/messagelist/api/IAwemeStatusViewModel;

    const/16 v3, 0x23

    if-eqz v1, :cond_9

    if-eqz v6, :cond_9

    invoke-virtual {v0, v1}, Lcom/ss/android/ugc/aweme/im/messagelist/api/IAwemeStatusViewModel;->hu2(Ljava/lang/String;)LX/03rU;

    move-result-object v0

    invoke-interface {v0}, LX/03rU;->getValue()Ljava/lang/Object;

    move-result-object v1

    instance-of v0, v1, LX/04dW;

    const-string v8, "available"

    if-eqz v0, :cond_3

    invoke-virtual {p0, p1, v8}, LX/0axc;->LJ(LX/0i9W;Ljava/lang/String;)V

    sget-object v0, Lcom/ss/android/ugc/aweme/im/service/service/IAwemeCoverManager;->LIZ:LX/08O5;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    invoke-static {}, LX/08O5;->LIZ()Lcom/ss/android/ugc/aweme/im/service/service/IAwemeCoverManager;

    move-result-object v2

    if-eqz v2, :cond_2

    iget-object v1, p0, LX/0axc;->LL:Landroid/content/Context;

    iget-boolean v0, p0, LX/0axc;->LLILL:Z

    invoke-interface {v2, v1, p1, v0}, Lcom/ss/android/ugc/aweme/im/service/service/IAwemeCoverManager;->LIZLLL(Landroid/content/Context;LX/0i9W;Z)LX/0n0z;

    move-result-object v7

    :cond_2
    new-instance v8, LX/0awt;

    iget-object v0, v5, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;->type:Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;

    invoke-virtual {p0, p1, v4, v0}, LX/0axc;->LIZJ(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;)LX/0ax9;

    move-result-object v2

    new-instance v1, LX/0am1;

    const/16 v0, 0x1cf

    invoke-static {v0}, Lkotlin/jvm/internal/AFwS237S0000000_17;->get$arr$(I)Lkotlin/jvm/internal/AFwS237S0000000_17;

    move-result-object v0

    invoke-direct {v1, v6, v7, v0}, LX/0am1;-><init>(Lcom/ss/android/ugc/aweme/base/model/UrlModel;LX/0n2V;Lkotlin/jvm/functions/Function1;)V

    iget-object v0, v5, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;->type:Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;

    invoke-static {p1, v4, v0}, LX/0axc;->LIZIZ(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;)LX/0avP;

    move-result-object v0

    invoke-direct {v8, v2, v1, v0, v3}, LX/0awt;-><init>(LX/0ax9;LX/0ax6;LX/0avM;I)V

    :goto_0
    new-instance v0, LX/0asl;

    invoke-direct {v0, v8}, LX/0asl;-><init>(Ljava/lang/Object;)V

    return-object v0

    :cond_3
    sget-object v0, LX/087P;->LIZ:LX/087P;

    invoke-static {v1, v0}, Lkotlin/jvm/internal/Intrinsics;->LJFF(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v0

    const-string v2, "unavailable"

    if-eqz v0, :cond_7

    invoke-virtual {p1}, LX/0i9W;->getLocalExt()Ljava/util/Map;

    move-result-object v1

    const-string v0, "ref_attachment_state"

    invoke-interface {v1, v0}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    if-eqz v0, :cond_4

    move-object v8, v0

    :cond_4
    invoke-static {v8, v2}, Lkotlin/jvm/internal/Intrinsics;->LJFF(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_5

    invoke-virtual {p0, p1, v4, v5}, LX/0axc;->LIZLLL(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;)LX/0awt;

    move-result-object v8

    goto :goto_0

    :cond_5
    sget-object v0, Lcom/ss/android/ugc/aweme/im/service/service/IAwemeCoverManager;->LIZ:LX/08O5;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    invoke-static {}, LX/08O5;->LIZ()Lcom/ss/android/ugc/aweme/im/service/service/IAwemeCoverManager;

    move-result-object v2

    if-eqz v2, :cond_6

    iget-object v1, p0, LX/0axc;->LL:Landroid/content/Context;

    iget-boolean v0, p0, LX/0axc;->LLILL:Z

    invoke-interface {v2, v1, p1, v0}, Lcom/ss/android/ugc/aweme/im/service/service/IAwemeCoverManager;->LIZLLL(Landroid/content/Context;LX/0i9W;Z)LX/0n0z;

    move-result-object v7

    :cond_6
    new-instance v8, LX/0awt;

    iget-object v0, v5, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;->type:Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;

    invoke-virtual {p0, p1, v4, v0}, LX/0axc;->LIZJ(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;)LX/0ax9;

    move-result-object v2

    new-instance v1, LX/0am1;

    const/16 v0, 0x1d0

    invoke-static {v0}, Lkotlin/jvm/internal/AFwS237S0000000_17;->get$arr$(I)Lkotlin/jvm/internal/AFwS237S0000000_17;

    move-result-object v0

    invoke-direct {v1, v6, v7, v0}, LX/0am1;-><init>(Lcom/ss/android/ugc/aweme/base/model/UrlModel;LX/0n2V;Lkotlin/jvm/functions/Function1;)V

    iget-object v0, v5, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;->type:Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;

    invoke-static {p1, v4, v0}, LX/0axc;->LIZIZ(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;)LX/0avP;

    move-result-object v0

    invoke-direct {v8, v2, v1, v0, v3}, LX/0awt;-><init>(LX/0ax9;LX/0ax6;LX/0avM;I)V

    goto :goto_0

    :cond_7
    instance-of v0, v1, LX/087N;

    if-eqz v0, :cond_8

    invoke-virtual {p0, p1, v2}, LX/0axc;->LJ(LX/0i9W;Ljava/lang/String;)V

    invoke-virtual {p0, p1, v4, v5}, LX/0axc;->LIZLLL(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;)LX/0awt;

    move-result-object v8

    goto :goto_0

    :cond_8
    new-instance v0, LX/0BB1;

    invoke-direct {v0}, LX/0BB1;-><init>()V

    throw v0

    :cond_9
    new-instance v9, LX/0asl;

    new-instance v8, LX/0awt;

    iget-object v0, v5, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;->type:Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;

    invoke-virtual {p0, p1, v4, v0}, LX/0axc;->LIZJ(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;)LX/0ax9;

    move-result-object v6

    sget-object v0, LX/0awo;->VIDEO:LX/0awo;

    invoke-virtual {v0}, LX/0awo;->toMediaUIState()LX/0ax3;

    move-result-object v5

    new-instance v4, LX/0avC;

    invoke-virtual {p1}, LX/0i9W;->getRefMsgId()J

    move-result-wide v0

    invoke-virtual {p1}, LX/0i9W;->getReferenceInfo()Lcom/bytedance/im/core/proto/ReferenceInfo;

    move-result-object v2

    if-eqz v2, :cond_a

    iget-object v7, v2, Lcom/bytedance/im/core/proto/ReferenceInfo;->ref_message_type:Ljava/lang/Long;

    :cond_a
    invoke-direct {v4, v0, v1, v7}, LX/0avC;-><init>(JLjava/lang/Long;)V

    invoke-direct {v8, v6, v5, v4, v3}, LX/0awt;-><init>(LX/0ax9;LX/0ax6;LX/0avM;I)V

    invoke-direct {v9, v8}, LX/0asl;-><init>(Ljava/lang/Object;)V

    return-object v9

    :cond_b
    sget-object v0, LX/0auq;->LIZ:LX/0auq;

    return-object v0
.end method

.method public final LIZJ(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;)LX/0ax9;
    .locals 5
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "LX/0i9W;",
            "Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;",
            "Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;",
            ")",
            "LX/0ax9<",
            "LX/05tN;",
            ">;"
        }
    .end annotation

    sget-object v0, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;->ATTACHMENT_TYPE_SHARE_COMMENT_MENTION:Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;

    if-ne p3, v0, :cond_0

    sget-object v0, LX/0axN;->LIZ:LX/0axN;

    return-object v0

    :cond_0
    sget-object v0, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;->ATTACHMENT_TYPE_REPLY_REPOST:Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;

    if-ne p3, v0, :cond_5

    invoke-static {p1}, LX/0b3L;->LJJIIZI(LX/0i9W;)Z

    move-result v0

    const/4 v4, 0x0

    if-eqz v0, :cond_4

    invoke-virtual {p1}, LX/0i9W;->getConversationId()Ljava/lang/String;

    move-result-object v0

    invoke-static {v0}, LX/0iMA;->LJI(Ljava/lang/String;)J

    move-result-wide v1

    sget-object v0, Lcom/ss/android/ugc/aweme/im/contacts/api/IMContactApi;->LIZ:LX/08Md;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    invoke-static {}, LX/08Md;->LIZ()Lcom/ss/android/ugc/aweme/im/contacts/api/IMContactApi;

    move-result-object v0

    invoke-interface {v0}, Lcom/ss/android/ugc/aweme/im/contacts/api/IMContactApi;->LJ()LX/11fw;

    move-result-object v3

    invoke-static {v1, v2}, Ljava/lang/String;->valueOf(J)Ljava/lang/String;

    move-result-object v2

    sget-object v0, Lcom/ss/android/ugc/aweme/im/contacts/api/service/IIMContactUtilService;->LIZ:LX/07vb;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    invoke-static {}, LX/07vb;->LIZ()Lcom/ss/android/ugc/aweme/im/contacts/api/service/IIMContactUtilService;

    move-result-object v1

    if-eqz v1, :cond_3

    invoke-virtual {p1}, LX/0i9W;->getConversationId()Ljava/lang/String;

    move-result-object v0

    invoke-interface {v1, v0}, Lcom/ss/android/ugc/aweme/im/contacts/api/service/IIMContactUtilService;->LJFF(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    :goto_0
    const-string v0, "im_chat_repost_msg"

    invoke-virtual {v3, v2, v1, v0}, LX/11fw;->LJIIIIZZ(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Lcom/ss/android/ugc/aweme/im/contacts/api/model/IMUser;

    move-result-object v0

    new-instance v3, LX/0axH;

    new-instance v2, LX/04f3;

    if-eqz v0, :cond_1

    invoke-virtual {v0}, Lcom/ss/android/ugc/aweme/im/common/model/BaseContact;->getNickName()Ljava/lang/String;

    move-result-object v4

    :cond_1
    invoke-static {v4}, LX/0MJU;->LIZ(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    if-nez v0, :cond_2

    const-string v0, ""

    :cond_2
    invoke-static {v0}, Ljava/util/Collections;->singletonList(Ljava/lang/Object;)Ljava/util/List;

    move-result-object v1

    const v0, 0x7f1225b7

    invoke-direct {v2, v0, v1}, LX/04f3;-><init>(ILjava/util/List;)V

    invoke-direct {v3, v2}, LX/0axH;-><init>(Ljava/lang/Object;)V

    return-object v3

    :cond_3
    move-object v1, v4

    goto :goto_0

    :cond_4
    new-instance v3, LX/0axH;

    new-instance v1, LX/04f3;

    const v0, 0x7f1225b8

    invoke-direct {v1, v0, v4}, LX/04f3;-><init>(ILjava/util/List;)V

    invoke-direct {v3, v1}, LX/0axH;-><init>(Ljava/lang/Object;)V

    return-object v3

    :cond_5
    invoke-virtual {p2}, Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;->getRefVideoIsStory()Z

    move-result v2

    invoke-static {p1}, LX/0axY;->LIZ(LX/0i9W;)Ljava/lang/Integer;

    move-result-object v1

    iget-object v0, p0, LX/0axc;->LL:Landroid/content/Context;

    invoke-static {v2, v1, p1, v0}, LX/0axY;->LIZJ(ZLjava/lang/Integer;LX/0i9W;Landroid/content/Context;)LX/0ax9;

    move-result-object v0

    return-object v0
.end method

.method public final LIZLLL(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;)LX/0awt;
    .locals 8

    invoke-virtual {p2}, Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;->getRefVideoIsStory()Z

    move-result v0

    if-eqz v0, :cond_1

    const v1, 0x7f1222e1

    :goto_0
    new-instance v7, LX/0awt;

    iget-object v0, p3, Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentComponent;->type:Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;

    invoke-virtual {p0, p1, p2, v0}, LX/0axc;->LIZJ(LX/0i9W;Lcom/ss/android/ugc/aweme/im/message/content/BaseContent$ReferenceVideo;Lcom/ss/android/ugc/aweme/im/message/template/component/AttachmentTypeComponent;)LX/0ax9;

    move-result-object v6

    new-instance v5, LX/04X1;

    new-instance v0, LX/04f3;

    const/4 v4, 0x0

    invoke-direct {v0, v1, v4}, LX/04f3;-><init>(ILjava/util/List;)V

    invoke-direct {v5, v0}, LX/04X1;-><init>(LX/04f3;)V

    new-instance v3, LX/0avC;

    invoke-virtual {p1}, LX/0i9W;->getRefMsgId()J

    move-result-wide v1

    invoke-virtual {p1}, LX/0i9W;->getReferenceInfo()Lcom/bytedance/im/core/proto/ReferenceInfo;

    move-result-object v0

    if-eqz v0, :cond_0

    iget-object v4, v0, Lcom/bytedance/im/core/proto/ReferenceInfo;->ref_message_type:Ljava/lang/Long;

    :cond_0
    invoke-direct {v3, v1, v2, v4}, LX/0avC;-><init>(JLjava/lang/Long;)V

    const/16 v0, 0x23

    invoke-direct {v7, v6, v5, v3, v0}, LX/0awt;-><init>(LX/0ax9;LX/0ax6;LX/0avM;I)V

    return-object v7

    :cond_1
    const v1, 0x7f1222e2

    goto :goto_0
.end method

.method public final LJ(LX/0i9W;Ljava/lang/String;)V
    .locals 3

    invoke-virtual {p1}, LX/0i9W;->getLocalExt()Ljava/util/Map;

    move-result-object v1

    const-string v0, "ref_attachment_state"

    invoke-interface {v1, v0}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    invoke-static {v0, p2}, Lkotlin/jvm/internal/Intrinsics;->LJFF(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_0

    return-void

    :cond_0
    new-instance v2, LX/03WT;

    const/4 v1, 0x0

    invoke-direct {v2, p1, p2, v1}, LX/03WT;-><init>(LX/0i9W;Ljava/lang/String;LX/02wT;)V

    const/4 v0, 0x3

    invoke-static {p0, v1, v1, v2, v0}, LX/15Ap;->LIZLLL(LX/02uK;Lkotlin/coroutines/CoroutineContext;LX/0PB8;Lkotlin/jvm/functions/Function2;I)LX/040L;

    return-void
.end method

.method public final getCoroutineContext()Lkotlin/coroutines/CoroutineContext;
    .locals 1

    iget-object v0, p0, LX/0axc;->LLILLIZIL:LX/0PBG;

    return-object v0
.end method
